...
// Array to store the conversation history with initial system message
let conversationHistory = [{
  role: "system",
  content: "You are a helpful beauty advisor specializing in skincare and beauty routines. When creating routines, explain the order of product application and why each product is used at that step. Keep explanations clear and beginner-friendly."
}];
...
generateRoutineButton.addEventListener("click", async () => {
  ...
  try {
    // Add the user's request to the conversation history with clear instructions
    conversationHistory.push({
      role: "user",
      content: `Create a detailed beauty routine using these products:\n${selectedProducts.map(product => 
        `- ${product.name} (${product.brand}) - ${product.category}\n  ${product.description}`
      ).join('\n')}\n\nPlease provide a step-by-step routine that explains:\n1. The order of application\n2. When to use each product (morning/evening)\n3. How to apply each product\n4. Why each product is used at that specific step`
    });
